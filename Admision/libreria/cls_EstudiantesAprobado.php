...
class Aprobados {
	...
	function guardarEditar(){
		...
		if($this->id > 0){
			...
			//nuevo
			$fecha = date('y/m/d');
			$sql= "INSERT INTO `estudianteaprobado`(`nombres`, `apellidos`, `fecha_nacimiento`, `nacionalidad`, `telefono`,
			`celular`, `direccion`, `estado_Civil`, `sexo`, `id_Tutor`, `fecha`, `id_Usuario`, `id_EstudianteViejo`) VALUES 
			('{$this->nombres}', '{$this->apellidos}', '{$this->fecha_nacimiento}', '{$this->nacionalidad}','{$this->telefono}',
			'{$this->celular}', '{$this->direccion}','{$this->estado_Civil}', '{$this->sexo}','{$this->id_Tutor}', '{$fecha}',
			'{$this->id_Usuario}','{$this->id_EstudianteViejo}')";
	...
	// function actualizarEstado(){
		// $sql = "UPDATE `estudiante` SET `estadoSolicitud` ='{$this->estadoSolicitud}' WHERE id = '{$this->id}'";
	...
	static function extraerEstudiantes(){
		$sql = "SELECT * from estudianteaprobado";
	...
	function traerAprobado(){
		$sql = "select * from estudianteaprobado where id = '{$this->id}'";
